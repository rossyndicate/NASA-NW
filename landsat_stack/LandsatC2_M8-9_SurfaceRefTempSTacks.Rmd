...
```{r}
py_env_dir = file.path(getwd(), 'landsat_stack/')
...
def RefPull89(image):
  ...
  #where dswe is equal to one, call it one, rename to dswe1 and mask with QA filters
  dswe1 = image.select(['Blue'],['dswe1']).updateMask(d.eq(1)).updateMask(f.eq(0)).updateMask(r.eq(1)).selfMask()
  # band where dswe is 3 and apply all masks
  dswe3 = image.select(['Blue'],['dswe3']).updateMask(d.eq(3)).updateMask(f.eq(0)).updateMask(r.eq(1)).selfMask()
  #calculate hillshade
  ...
  pixOut = (image.select(['Aerosol', 'Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2', 'SurfaceTemp', 'temp_qa'])
            ...
            .updateMask(r.eq(1)) #no saturated
            .updateMask(f.eq(0)) #no clouds or snow
            .addBands(clouds) #after masking, add clouds
            .addBands(dswe1) #add dswe1
  ...
  .combine(ee.Reducer.count().unweighted().forEachBand(pixOut.select(['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','SurfaceTemp', 'dswe', 'dswe1', 'dswe3', 'clouds'])), outputPrefix = 'pCount_', sharedInputs = False)
  #.combine(ee.Reducer.sum().unweighted().forEachBand(pixOut.select(['dswe1', 'dswe3'])), outputPrefix = 'pCount_', sharedInputs = False)
  .combine(ee.Reducer.mean().unweighted().forEachBand(pixOut.select(['hillShadow', 'medHighAero'])), outputPrefix = 'prop_', sharedInputs = False))
...
for tiles in pr:
  ...
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'kurt_SurfaceTemp', 'prop_clouds','prop_medHighAero','prop_hillShadow','pCount_dswe', 'pCount_dswe1', 'pCount_dswe3', 'min_cloud_dist', 'mean_cloud_dist','system:index']))
...
for tiles in pr:
  ...
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'sd_Aerosol','sd_Blue', 'sd_Green', 'sd_Red', 'sd_Nir', 'sd_Swir1', 'sd_Swir2', 'sd_SurfaceTemp', 'sd_temp_qa',
                                          'pCount_Aerosol','pCount_Blue', 'pCount_Green', 'pCount_Red', 'pCount_Nir', 'pCount_Swir1', 'pCount_Swir2', 'pCount_SurfaceTemp',
                                          'kurt_SurfaceTemp', 'pCount_clouds','prop_medHighAero','prop_hillShadow','pCount_dswe1', 'pCount_dswe3', 'min_cloud_dist', 'mean_cloud_dist','system:index']))
