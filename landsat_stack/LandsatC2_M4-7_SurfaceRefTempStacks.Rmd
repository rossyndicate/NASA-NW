...
#save info locally
write.csv(st_drop_geometry(all_lakes), (file.path(data_dir, 'NHDPlus_stats_NWlakes.csv')), row.names = F)

all_lakes = all_lakes %>% select(comid, gnis_name)

st_write(all_lakes, file.path(data_dir, 'NHDPlus_NWlakes.shp'))
```
...
def DSWE(i):
  ...
  # The folowing thresholds are taken from the LS Collection 2 DSWE Data Format Control Book
...
def RefPull457(image):
  ...
  dswe1 = d.eq(1).rename('dswe1').updateMask(f.eq(0)).updateMask(s.eq(0)).updateMask(r.eq(1)).selfMask()
  # band where dswe is 3 and apply all masks
  dswe3 = d.eq(3).rename('dswe3').updateMask(f.eq(0)).updateMask(s.eq(0)).updateMask(r.eq(1)).selfMask()
  #calculate hillshade
